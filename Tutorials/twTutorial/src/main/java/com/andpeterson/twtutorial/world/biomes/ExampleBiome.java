package com.andpeterson.twtutorial.world.biomes;
...
public class ExampleBiome extends Biome {
	...
	public ExampleBiome(Builder biomeBuilder) {
		super(biomeBuilder);
		//weight, min in group, max in group
		...
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.NORMAL_TREE.withConfiguration(JazzTree.JAZZ_TREE_CONFIG).withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(7, 0.1f, 1))));
...
// gradlew --refresh-dependencies (after updating mappings)